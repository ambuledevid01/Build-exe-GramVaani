...
serve(async (req) => {
  ...
  try {
    ...
    // Use a multilingual voice that supports Hindi - default to "Rachel" which has good Hindi support
    const selectedVoiceId = voiceId || "21m00Tcm4TlvDq8ikWAM"; // Rachel voice - good for Hindi
    ...
    const response = await fetch(
      `https://api.elevenlabs.io/v1/text-to-speech/${selectedVoiceId}?output_format=mp3_44100_128`,
      {
        ...
        body: JSON.stringify({
          text,
          model_id: "eleven_multilingual_v2", // Supports Hindi and other languages
          voice_settings: {
            stability: 0.5,
